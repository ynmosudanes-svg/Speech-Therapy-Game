...
import { ArrowRight, Eye, EyeOff, Save, UserCheck, Check, UsersRound } from 'lucide-react';
import { useTherapyStore } from '../../hooks/useTherapyStore';
import { parentService } from '../../services/parentService';

const ParentForm = ({ mode = 'create' }) => {
  ...
  useEffect(() => {
    const fetchParent = async () => {
      ...
      try {
        ...
        const parent = Array.isArray(response?.data)
          ? response.data.find((item) => item.id === parentId)
          : null;

        if (!parent) {
          setError('لم يتم العثور على ولي الأمر.');
          return;
        }
        ...
        setError('حدث خطأ أثناء جلب بيانات ولي الأمر.');
      } finally {
        setLoading(false);
      }
    };
    ...
    fetchParent();
  }, [adminSession?.token, mode, parentId]);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (mode === 'create') {
        ...
        await parentService.createParent(adminSession.token, formData);
      } else {
        ...
        await parentService.updateParent(adminSession.token, parentId, payload);
      }
      ...
      <div className="flex items-center justify-between mb-8 border-b border-slate-100 pb-6">
        <div>
          <h1 className="text-3xl font-black text-slate-900">
            {mode === 'create' ? 'إضافة حساب لولي أمر' : 'تعديل بيانات حساب ولي الأمر'}
          </h1>
          <p className="text-slate-500 mt-2 text-sm md:text-base">
            {mode === 'create' 
              ? 'قم بإدخال البيانات الأساسية لإرسال دعوة الدخول ومتابعة الأطفال.'
              : 'تحديث معلومات ولي الأمر وكلمة المرور.'}
          </p>
        </div>
        <button 
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-[2rem] p-6 md:p-10 shadow-lg shadow-slate-200/40 border border-slate-100 space-y-8 relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
          <div className="space-y-2">
            <label className="block text-sm font-bold text-slate-700 ml-1">اسم ولي الأمر</label>
            <input 
              type="text" 
              name="name"
              value={formData.name}
        ...
        <div className="space-y-4 pt-6 border-t border-slate-100 relative z-10">
          <label className={`flex items-center justify-between p-6 rounded-2xl border-2 cursor-pointer transition-all duration-300 ${formData.isActive ? 'border-[#178bb6] bg-gradient-to-l from-cyan-50/50 to-white shadow-sm' : 'border-slate-100 bg-slate-50/50 hover:bg-slate-50 hover:border-slate-200'}`}>
            <div className="flex-1 pr-2">
              ...
              <p className="text-sm text-slate-500 mt-2 leading-relaxed">يمكن لولي الأمر تسجيل الدخول ومتابعة أطفاله إذا كان الحساب نشطاً.</p>
            </div>
        ...
        <div className="pt-8 border-t border-slate-100 flex justify-end relative z-10">
          ...
          >
            <Save size={22} />
            {submitting ? 'جارٍ الحفظ...' : (mode === 'create' ? 'حفظ الحساب' : 'حفظ التعديلات')}
          </button>
        </div>
